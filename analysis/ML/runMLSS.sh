  ...
  do
    ...
    do
      ...
      do
		...
		Rscript new.dating.R --tree treesSS/${name}.rooted --info infoSS/${name}.csv --timetree outfilesSS/${name}.timetree --data outfilesSS/${name}.data.csv --stats outfilesSS/${name}.stats.csv --patid ${name} &> outfilesSS/${name}out &
        done
      done
    done
    wait $!
